	...
	graded_lp_cat_sym := GradedLeftPresentations( S : FinalizeCategory := false );
	  ...
	  if not HasIsFinalized( graded_lp_cat_sym ) then 
	    ...
	    AddEvaluationMorphismWithGivenSource( graded_lp_cat_sym,
	      function( a, b, s )
	        local mor;
	        mor := EvaluationMorphismWithGivenSource( 
            UnderlyingPresentationObject( a ), 
            UnderlyingPresentationObject( b ), 
            UnderlyingPresentationObject( s ) );
      ...
      AddCoevaluationMorphismWithGivenRange( graded_lp_cat_sym,
	      function( a, b, r )
	        local mor;
	        mor := CoevaluationMorphismWithGivenRange( 
            UnderlyingPresentationObject( a ),
            UnderlyingPresentationObject( b ),
            UnderlyingPresentationObject( r ) );
...
InstallMethodWithCrispCache( HOMALG_GRADED_POLYNOMIAL_RING, 
    ...
    if IsBound( WITH_MAGMA ) and ValueGlobal( "WITH_MAGMA" ) in [ 'y', 'Y', true ] then
